...
| `requirements.txt` | `mcp[cli]`, `pytermgui`. |
| `setup.bat` / `run.bat` | Install deps / launch the TUI. |

## Active work

A pytermgui TUI control panel is under construction. **Before editing `tui.py`, read [HANDOFF.md](HANDOFF.md)** — it documents the completed bugfixes (ModelButton wrapping & highlights), the REST API for retrieving user quota info, and the redesign requirements (adding left sidebar, removing box A, showing models & quota info).

## Conventions

- **Platform is Windows.** Python 3.10+ (dev machine runs 3.14). Paths use backslash; the dev shell is PowerShell.
- **TUI = pytermgui**, chosen for minimal disk footprint (only `wcwidth`). Do not swap to Textual/Rich without asking the owner.
- **Frame colour is uniform grey `240`.**
- **Smoke-test the TUI without a TTY:**
  `PYTHONIOENCODING=utf-8 python -c "import tui; tui.build_window()"`.
  Set `PYTHONIOENCODING=utf-8` for anything that prints box-drawing characters (Windows cp1252 otherwise crashes).
- `run.bat` launches the **TUI**, not the server. The server is launched by the MCP client config (`python server.py`).
...
- The owner iterates visually via screenshots and wants a **minimalist** UI ("越精简越好" — the simpler the better). Prefer flat, low-chrome widgets.
