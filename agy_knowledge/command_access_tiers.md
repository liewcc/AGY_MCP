...
| **E** | 本地 gRPC 语言服务器 | 中 | ⏳ 1/3（usage 已完成） |
...
| 斜线指令 | gRPC 方法 | 状态 |
|----------|-----------|------|
| `/usage` | `RetrieveUserQuotaSummary` | ✅ 已实现（见 AGENTS.md "Method E"） |
| `/tasks` | 待探索 | ⏳ |
| `/agents` | 待探索 | ⏳ |

完整技术细节（端口发现、TLS 证书提取、protobuf 解码）见
[../AGENTS.md](../AGENTS.md) 的 "How the gRPC quota method works"。
实现位置：`agy_models.get_quota_summary()`。
...
- **已实现工具：22 个**（Tier A 原有 + B/C/D 新增 19 个）
- **已验证**：MCP server 重启后正确加载，真实调用读写均正常
- **剩余**：Tier E 的 tasks/agents（gRPC 探索），Tier F 全部（需 PTY 封装）

代码文件：`agy_client.py`、`conversations.py`、`tier_c_commands.py`、
`tier_d_commands.py`、`agy_models.py`，统一在 `server.py` 注册为 MCP 工具。
